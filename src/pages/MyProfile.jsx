...
import { getUserProfile, getUserBadges, getUserChallengeHistory } from '../api/exportAllApi';
...
function MyProfile() {
  ...
  const getLastChallenge = () => {
    ...
    const completed = challengeHistory.filter(ch => ch.status === 'completed');
  ...
  const getRecentAchievement = () => {
    ...
    return badges.sort((a, b) => new Date(b.earnedAt) - new Date(a.earnedAt))[0];
  };
    ...
    <div className="flex flex-col md:flex-row">
      ...
      <div className="bg-primary text-primary-content w-full md:w-64 p-6 flex flex-col items-center">
        ...
        <p className="text-sm text-primary-content opacity-70 mb-4">
          joined: {profile?.createdAt ? new Date(profile.createdAt).toLocaleDateString() : '[Date]'}
        </p>
        
        {/* Edit Profile Button */}
        <Link 
          to="/settings" 
          className="btn btn-accent btn-block mb-8"
        >
          Edit Profile
        </Link>
        ...
        <div className="w-full mb-6">
          <div className="flex justify-between items-center mb-1">
            ...
            <span className="text-xs text-primary-content opacity-70">{profile?.experience || 0}/100 XP</span>
          </div>
          <progress 
      ...
      <div className="flex-grow p-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-10">
          <h1 className="text-xl font-medium mb-4 md:mb-0">
            Welcome back, {profile?.username || user?.username || '[Username]'}! Are you ready for today's challenge?
          </h1>
                  ...
                  <div className="avatar">
                    <div className="w-14 rounded-full bg-accent-focus text-accent-content flex items-center justify-center">
                      {recentAchievement.icon ? (
                        <img src={recentAchievement.icon} alt={recentAchievement.name} />
                      ) : (
                        <Medal size={24} />
                      )}
                    </div>
                  </div>
                  <div className="mt-2 font-medium">{recentAchievement.name || 'Fitness Beginner'}</div>
